...
def animate(i):
    ...
    # # Apply Gaussian filter to the audio data
    # filtered_data = gaussian_filter1d(data_int_centered, sigma=1.0)
    
    # Apply moving average filter to the audio data
    window_size = 2  # Adjust the window size as needed
    moving_avg_data = np.convolve(data_int_centered, np.ones(window_size) / window_size, mode='same')
    ...
    ax2.set_title(f'FFT - Strongest Frequency: {frequency:.2f} Hz')
    ax3.clear()
    ax4.clear()
